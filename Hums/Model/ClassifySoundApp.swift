...
struct AppConfiguration {
    ...
    var monitoredSounds : Set<SoundIdentifier> = [SoundIdentifier(labelName: "humming")]
...
class AppState: ObservableObject {
    ...
    static func advanceDetectionStates(_ oldStates: [(SoundIdentifier, DetectionState)],
        ...
        let selectedSound = "humming" // the label here is modified to only detect humming sounds

        let confidenceForLabel = { (sound: SoundIdentifier) -> Double in
            guard sound.labelName == selectedSound else { return 0 } // Ignoring any other possible sounds
            if let classification = result.classification(forIdentifier: selectedSound) {
...
//    /// Asking for user permission to access the microphone
//    private var audioSession = AVAudioSession.sharedInstance()
//    func requestMicrophonePremission(){
//        audioSession.requestRecordPermission { granted in
//            DispatchQueue.main.async {
//                if granted {
//                    print("Microphone acess granted")
//                } else {
